...
import api from '../../api'; // Pastikan file api.js sudah benar
...
export default function Login() {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        try {
            ...
            // Ini PENTING agar Navbar bisa menampilkan nama asli user
            localStorage.setItem('auth_token', access_token);
            localStorage.setItem('user_info', JSON.stringify(user)); 

            // D. Logika Redirect Berdasarkan Role
            if (user.role === 'guru' || user.role === 'admin') {
                navigate('/dashboard-guru');
            } else {
                navigate('/dashboard'); // Default untuk siswa
            }
            ...
            // E. Handle Error dari Backend
            if (error.response && error.response.status === 401) {
                setErrorMsg("Email atau Password salah. Silakan coba lagi.");
            } else if (error.response && error.response.data.message) {
                setErrorMsg(error.response.data.message);
            } else {
                setErrorMsg("Terjadi kesalahan koneksi. Pastikan server backend menyala.");
            }
        }
    };
                ...
                <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-8 lg:p-12 z-10 bg-white h-full overflow-y-auto">
                    <div className="w-full max-w-md mt-16 lg:mt-0"> 
                        ...
                        <div className="text-center mb-8">
                            ...
                            <p className="text-sm text-gray-500">Silakan masuk menggunakan email Anda.</p>
                        </div>

                        {errorMsg && (
                            <div className="bg-red-50 text-red-600 px-4 py-3 rounded-xl text-sm mb-6 border border-red-200 text-center animate-pulse flex items-center justify-center gap-2">
